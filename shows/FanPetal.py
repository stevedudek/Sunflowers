...
class FanPetal(object):
	...
	def draw_fan(self):
		for s in range(NUM_SUNFLOWERS):
			for p in self.sunflower.get_petal_sym(self.fan_sym + s, self.sunflower.get_num_spirals() - 1 - (self.clock % self.sunflower.get_num_spirals())):
	...
	def draw_petal(self):
		for s in range(NUM_SUNFLOWERS):
			for p in self.sunflower.get_petal_sym(self.petal_sym + s, self.clock % self.sunflower.get_num_spirals()):
