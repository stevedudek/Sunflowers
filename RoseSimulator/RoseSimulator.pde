...
byte[][][] curr_buffer = new byte[numBigRose][NUM_PIXELS][3];
byte[][][] next_buffer = new byte[numBigRose][NUM_PIXELS][3];
byte[][][] morph_buffer = new byte[numBigRose][NUM_PIXELS][3];
...
void processPixelCommand(String cmd) {
  ...
  sendColorOut(rose, pix, byte(r), byte(g), byte(b), false);  
...
void sendColorOut(byte rose, int pix, byte r, byte g, byte b, boolean morph) {
  ...
  if (TILING) {
    ...
    setPixelBuffer(rose, pix, r, g, b, morph);  // Lights: sets next-frame buffer (doesn't turn them on)
  } else {
    if (rose == 0) {
      for (byte rose_num = 0; rose_num < numBigRose; rose_num++) {
        setCellColor(color(r,g,b), rose_num, pix);  // Simulator
        setPixelBuffer(rose_num, pix, r, g, b, morph);  // Lights: sets next-frame buffer (doesn't turn them on)
      }
    }
  }
}
...
void setPixelBuffer(byte BigRose, int pixel, byte r, byte g, byte b, boolean morph) {
