...
short[][][] past_buffer = new short[NUM_SUNFLOWERS][NUM_LEDS][3];
    ...
    for (byte s = 0; s < NUM_SUNFLOWERS; s++) {
      for (short i = 0; i < NUM_LEDS; i++) {
        ...
        if (strip_num < strip_array.length) {
          strip_array[strip_num].setPixel(getPixelBuffer(s,i), pixel);
        } else {
...
void debug_lights() {
  println(millis() + ", " + get_overall_brightness());
  /*
  print(millis() + ", " + morph_buffer[0][0][0]);
  println(", " + morph_buffer[0][21][0]);
  
  // Track the red part of just one pixel (0, 10)
  short r = morph_buffer[0][10][0];  // red of (0, 10)
  if (true) {
    track_flag = true;
    println(millis() + ", " + r);
    return;
...
long get_overall_brightness() {
  long total = 0;
  for (byte s = 0; s < NUM_SUNFLOWERS; s++) {
    for (int i = 0; i < NUM_LEDS; i++) {
      for (int j = 0; j < 3; j++) {
        total += morph_buffer[s][i][j];
      }
    }
  }
  return total;
...
//  morph frame
//
//  3-State morphing = BLUR!
//
//  Fractional morphing between previous, current and next frame
//  State has a "turn on" slope of +0.5
//  and "turn off" slope of -0.33
//
...
void morph_frame(float fract) {
  ...
  float past_fract = (1.0 - fract) * 0.5;
  float curr_fract = ((1.0 - fract) * 0.5) + 0.5;
  float next_fract = fract;
  
  for (byte s = 0; s < NUM_SUNFLOWERS; s++) {
    for (int i = 0; i < NUM_LEDS; i++) {
      
      r = ((past_buffer[s][i][0] * past_fract) +
           (curr_buffer[s][i][0] * curr_fract) +
           (next_buffer[s][i][0] * next_fract)) / 1.5;
      
      g = ((past_buffer[s][i][1] * past_fract) +
           (curr_buffer[s][i][1] * curr_fract) +
           (next_buffer[s][i][1] * next_fract)) / 1.5;
      
      b = ((past_buffer[s][i][2] * past_fract) +
           (curr_buffer[s][i][2] * curr_fract) +
           (next_buffer[s][i][2] * next_fract)) / 1.5;
      
      sendColorOut(s,i, (short)r, (short)g, (short)b, true);
    }
  }
}  
...
void pushColorBuffer() {
  for (byte s = 0; s < NUM_SUNFLOWERS; s++) {
    for (int i = 0; i < NUM_LEDS; i++) {
      past_buffer[s][i][0] = curr_buffer[s][i][0];
      past_buffer[s][i][1] = curr_buffer[s][i][1];
      past_buffer[s][i][2] = curr_buffer[s][i][2];
