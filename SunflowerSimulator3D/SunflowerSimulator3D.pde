...
short[][][][] curr_buffer = new short[NUM_SUNFLOWERS][SPIRAL_COUNT][MAX_DIST][3];
short[][][][] next_buffer = new short[NUM_SUNFLOWERS][SPIRAL_COUNT][MAX_DIST][3];
short[][][][] morph_buffer = new short[NUM_SUNFLOWERS][SPIRAL_COUNT][MAX_DIST][3];
...
void draw() {
  background( 255);
  camera(width/2, height/2, 1.5 * height, // (height/2) / tan(PI/6), 
...
class SunForm {
  ...
  void draw_black_circle() {
    ...
    fill(0,0,0);
    stroke(0);
    ellipse((width / 2), ((height - CONTROL_HEIGHT) / 2), SUNFLOWER_RAD, SUNFLOWER_RAD);
  }
}
...
void processPixelCommand(String cmd) {
  ...
  sendColorOut(s, p, d, (short)r, (short)g, (short)b, false);  
...
void sendColorOut(byte s, int p, int d, short r, short g, short b, boolean morph) {
...
void setPixelBuffer(byte s, int p, int d, short r, short g, short b, boolean morph) {
  if (morph) {
